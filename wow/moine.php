<html>
 ...
 <body style = "background-color:#191410" >
		...
		<img id="class_img" src = "images/classes/moine.png">
	  <?php include 'connect.php' ?>
	  
	  <div id ="class_txt">
	  <div id="nom_class"> 
<?php $reqCat=$bd->prepare('SELECT * FROM classes WHERE id=11');
$reqCat->execute();
...
<?php $reqCat=$bd->prepare('SELECT extensions.nom FROM extensions, classes WHERE extensions.id =classes.id_extensions and classes.id=11');
$reqCat->execute();
$reqCat->setFetchMode(PDO::FETCH_OBJ);
while ($result=$reqCat->fetch() ) {
 echo '<p>',$result->nom;
...
<?php $reqCat=$bd->prepare('SELECT races.nom FROM races, races_classes WHERE races.id =races_classes.id_races and races_classes.id_classes=11');
$reqCat->execute();
$reqCat->setFetchMode(PDO::FETCH_OBJ);
while ($result=$reqCat->fetch() ) {
 echo '<p>',$result->nom;
